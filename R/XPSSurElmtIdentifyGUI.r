...
#'
#'
#'@param CurrentXPSSample XPSSample name
#'@param SpectIdx  XPSCoreline index
#'
#'@export 
#'


XPSSurveyElementIdentify <- function(CurrentXPSSample, SpectIdx)  {
...
#---- Var-Initialization
   ...
   Object <- get(CurrentXPSSample, envir = .GlobalEnv)
   nameCtrl <- Object[[SpectIdx]]@Symbol
   if (nameCtrl != "survey") {
   ...
   if ( is.numeric(SpectIdx) ) {
   	newcoreline <- Object[[SpectIdx]] # duplicate CoreLine
   } else if ( is.character(SpectIdx) ) {
   	indx <- which(SpectIdx == names(Object), arr.ind=TRUE)
